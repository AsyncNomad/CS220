        ...
        Node::NonLeaf(child) => {
            ...
            for element in buffer {
                match element {
                    Node::NonLeaf(child) => {
                        result.push(child.0);
                        let mut buffer1 = child.1;
                        for element in buffer1 {
                            match element {
                                Node::NonLeaf(child) => {
                                    result.push(child.0);
                                    let mut buffer1 = child.1;
...
pub fn du_sort(root: &File) -> Vec<(&str, usize)> {
    ...
    // sorted_by
    fn dissolve<'a>(directory: &'a Vec<File>, result: &mut Vec<(&'a str, usize)>) -> usize {
        let mut sum: usize = 0;
        for element in directory {
            match element {
                File::Directory(name, vec) => {
                    let buffer = dissolve(vec, result);
                    sum += buffer;
    ...
    match root {
        File::Directory(name, vec) => {
            let buffer = dissolve(vec, &mut result);
            result.push((name, buffer));
        }
        File::Data(name, size) => {
            result.push((name, *size));
